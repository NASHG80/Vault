...
export default function LenderHistory() {
  ...
  const summaryStats = [
    { label: t('history.totalPaid'), value: `₹${totalPaid.toLocaleString("en-IN")}`, sub: `${approved.length} loan${approved.length !== 1 ? "s" : ""} approved`, icon: Wallet },
    { label: t('hirer.jobsDone'), value: records.length, sub: "Total applications reviewed", icon: Receipt },
    { label: t('lender.withCertificates'), value: uniqueWorkers, sub: "Distinct applicants", icon: Users },
    { label: t('lender.avgTrustScore'), value: avgAmount > 0 ? `₹${Math.round(avgAmount).toLocaleString("en-IN")}` : "—", sub: "Per approved loan", icon: TrendingUp },
  ];
      ...
      <section className="max-w-6xl mx-auto px-8 pt-8">
        ...
        <motion.header initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <div className="flex items-center gap-2 mb-4">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black text-white text-[10px] font-black tracking-widest uppercase">
              <Receipt className="w-3 h-3" /> {t('lender.portalVersion')}
            </span>
          </div>
          <div className="flex items-end justify-between gap-4">
            <div>
              <h1 className="text-4xl md:text-5xl font-bold tracking-tighter mb-3 leading-tight">
                {t('history.paymentHistory')}
              </h1>
            ...
            <button onClick={fetchHistory} disabled={loading}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-xs font-bold hover:bg-slate-50 transition-all disabled:opacity-50 flex-shrink-0">
              <RefreshCw size={13} className={loading ? "animate-spin" : ""} /> {t('common.retry')}
            </button>
          </div>
        </motion.header>
        ...
        >
          {summaryStats.map((stat) => (
            <div key={stat.label} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{stat.label}</p>
                <div className="w-7 h-7 bg-[#f3f3f4] rounded-lg flex items-center justify-center">
          ...
          {statuses.map((s) => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-4 py-2 rounded-xl text-xs font-bold capitalize transition-all ${
                filter === s ? "bg-black text-white" : "bg-white border border-slate-200 text-slate-500 hover:text-black hover:border-black/20"
              }`}>
        ...
        {!loading && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
            className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_40px_rgba(0,0,0,0.04)] overflow-hidden mb-12">
            ...
            <div className="divide-y divide-slate-50">
              {filtered.map((record, idx) => {
                ...
                const actionedDate = record.actioned_at
                  ? new Date(record.actioned_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })
                ...
                return (
                  <motion.div
                    ...
                    className="grid grid-cols-[2fr_2fr_1fr_1fr_1fr] gap-4 px-8 py-5 items-start hover:bg-[#fafafa] transition-colors"
                  >
                    {/* Worker */}
                      ...
                      <div className="min-w-0">
                        ...
                        {trustScore ? (
                          <span className="text-[9px] font-black text-slate-500 flex items-center gap-0.5">
                            <BadgeCheck size={9} className="text-emerald-600" /> Trust: {trustScore}/99
                          </span>
                        ) : (
                          <span className="text-[9px] text-amber-500 font-medium flex items-center gap-1">
                            <AlertTriangle size={9} /> No cert
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    {/* Amount */}
                    <div>
                      <p className="text-sm font-black tracking-tight text-black">₹{Number(record.amount).toLocaleString("en-IN")}</p>
                      {record.certificate_cid && (
                        <a href={`https://gateway.pinata.cloud/ipfs/${record.certificate_cid}`} target="_blank" rel="noreferrer"
            ...
            {filtered.length === 0 && (
              <div className="py-20 text-center">
                ...
                <p className="text-slate-400 text-sm mt-1">
                  {filter === "All" ? "Approve or reject loan applications from the Dashboard to see them here." : "Try a different filter."}
                </p>
              </div>
            )}
          </motion.div>
        ...
        {/* ─── Footer ─── */}
        <footer className="border-t border-slate-100 pt-8 pb-12 flex flex-col md:flex-row justify-between items-start gap-6 opacity-70 hover:opacity-100 transition-opacity">
          <div className="max-w-md">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-3">{t('lender.complianceTitle')}</p>
            <p className="text-sm text-slate-500 font-medium leading-relaxed">
              {t('lender.complianceDesc')}
            </p>
          </div>
          <div className="text-right">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Ledger Version</p>
            <p className="text-sm font-black font-mono text-black">v2.1.0-LND</p>
          </div>
        </footer>
      </section>
    </LenderLayout>
